...
def _check_error_list(results: _EntrezSearchResult) -> None:
    """Refuse a search NCBI rejected inside a 200.
    ...
    one OR-branch matching nothing while the rest of the query answers.
    The first arrived with ``Count`` 0, which read as "no new papers" and
    a green run for as long as the tag stayed wrong; the second is logged.
    """
