...
_SPACES: Final[re.Pattern[str]] = re.compile(r" {2,}")


def _fold(text: str) -> str:
    """Lower-case ASCII words and single spaces, the form conditions meet in.

    An accent is dropped rather than split on: "Behçet's Disease" read as
    "beh et s disease", which no configured "behcet" could match -- and 39
    of the 137 studies a Behçet's search returned state only the accented
    form. A run of spaces is one, so "Stroke, Lacunar" reads "stroke
    lacunar". Applied to the configured terms too, so an accented one meets
    the same form.
    """
    decomposed = unicodedata.normalize("NFKD", text)
    bare = "".join(c for c in decomposed if not unicodedata.combining(c))
    return _SPACES.sub(" ", _CONDITION_NOISE.sub(" ", bare.lower()))


_FOLDED_CONDITIONS: Final[tuple[str, ...]] = tuple(_fold(t) for t in _CONDITIONS)
_FOLDED_PAIRS: Final[tuple[tuple[str, str], ...]] = tuple(
    (_fold(a), _fold(b)) for a, b in _CONDITION_PAIRS
)
...
def is_disease_study(study: dict[str, Any]) -> bool:
    ...
    for condition in conditions:
        normalised = _fold(condition)
        if any(term in normalised for term in _FOLDED_CONDITIONS):
            return True
        if any(a in normalised and b in normalised for a, b in _FOLDED_PAIRS):
            return True
    return False
