...
def _paper_results_to_summaries(
    ...
    # serialised as the string "0.85" instead of a float -- so the run
    # report this function builds would carry quoted numbers, and anything
    # reading it back (a diff between two runs, a chart, a jq query) sees a
...
def summarize_token_usage(tu: TokenUsage, cost: float) -> dict[str, Any]:
    """The report's token block, from one run's accumulated usage.

    Shared with the failed-run path in main.py, which used to publish an
    empty block -- zero tokens and $0.00 -- for a run whose spend it had in
    hand.
    """
    return {
        ...
        "estimated_cost_usd": _round_cost(cost),
    }
...
def _build_common_run_data(
    ...
    *,
    ...
    cost = client.estimate_cost(tu, batched=batched)
    ...
    return {
        ...
        "token_usage": summarize_token_usage(tu, cost),
...
def _build_offline_run_data(
    ...
    data["papers"]["total"] = data["papers"]["failed"] + metrics.papers_processed
    return data
...
def _overview_lines(data: PipelineRunData) -> list[str]:
    ...
    configured_mode = cfg.get("mode")
    run_mode = configured_mode if isinstance(configured_mode, str) else None
    mode_labels: dict[str | None, str] = {
        "local_pdf": "LOCAL PDF",
        "pmid_list": "PMID LIST",
    }
    mode = mode_labels.get(
        run_mode,
        "DRY RUN" if cfg.get("dry_run") else "LIVE",
    )
...
def _timing_breakdown(paper: PaperSummary) -> str:
    ...
    timings = (
        ("pdf", paper.get("pdf_parse_time", 0.0)),
        ("llm", paper.get("llm_time", 0.0)),
        ("val", paper.get("validation_time", 0.0)),
    )
    parts = [f"{label}:{duration:.1f}s" for label, duration in timings if duration > 0]
    return " ".join(parts)
...
def _print_database_panel(console: Console, data: PipelineRunData) -> None:
    ...
    A missing or None ``database`` means *dry run*, and nothing else: a real
    run that merged no genes still reports zeroed counts, because it still
    recorded its processed PMIDs. See ``_merge_processed_batch``.
    """
