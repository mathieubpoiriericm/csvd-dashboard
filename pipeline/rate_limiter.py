...
class AsyncRateLimiter:
    ...
    def __init__(self, rpm: int = 50, tpm: int = 100_000) -> None:
        self._rpm = rpm
        self._tpm = tpm
        # One entry per request. Its timestamp is both the RPM window and the
        # key for the TPM reservation, so a second timestamp deque would be
        # duplicate state that has to be pruned in lockstep with this one.
    ...
    async def acquire(self, estimated_tokens: int = 2000) -> int:
        ...
        Raises:
            RuntimeError: If the estimate exceeds the TPM limit outright.
                With an empty window nothing ages out, so such a request
                could never be admitted and the loop below would wait for
                it silently, forever. PipelineConfig refuses the pair at
                startup; this is the guard for a direct caller.
        """
        if self._tpm > 0 and estimated_tokens > self._tpm:
            raise RuntimeError(
                f"A request estimated at {estimated_tokens} tokens can never be "
                f"admitted under a {self._tpm} tokens-per-minute limit"
            )
        while True:
            async with self._lock:
                ...
                if now < self._global_backoff_until:
                    ...
                    rpm_ok = self._rpm == 0 or len(self._token_log) < self._rpm
                    tpm_ok = (
                        self._tpm == 0
                    ...
                    if self._token_log and self._rpm > 0:
                        sleep_time = max(0.05, self._token_log[0][0] + 60.0 - now)
