"""Shared cache, batching, and external-data sync utilities."""
...
class SyncResult:
    ...
    ``errors`` is per item -- one string for each gene, ORPHAcode or
    study the sync could not fetch -- and a sync that collected some is
    still a sync that carried on and wrote what it could.
    ``aborted`` is the other thing: the sync stopped. Only that makes the
    refresh red, so keep the two apart. See ``_result_summary`` in
    ``pipeline/main.py``.
    """
...
async def sync_cache_misses[T](
    keys: list[str],
    *,
    cached_count: int,
    fetch_batch: Callable[[list[str]], Awaitable[list[T]]],
    upsert_batch: Callable[[list[T]], Awaitable[Any]],
    is_success: Callable[[T], bool],
    error_for: Callable[[T], str],
    is_cacheable_miss: Callable[[T], bool] | None = None,
) -> SyncResult:
    """Fetch missing keys, persist outcomes, and return their shared counts.

    Successful rows and confirmed negative-cache rows are written separately,
    matching the existing database sync contract. Transient failures are
    reported but never persisted.
    """
    if not keys:
        return SyncResult(cached=cached_count)

    records = await fetch_batch(keys)
    successful: list[T] = []
    failed: list[T] = []
    ...
    if successful:
        await upsert_batch(successful)
    if is_cacheable_miss is not None:
        cacheable_misses = [record for record in failed if is_cacheable_miss(record)]
        if cacheable_misses:
            await upsert_batch(cacheable_misses)

    return SyncResult(
        fetched=len(successful),
        cached=cached_count,
        failed=len(failed),
        errors=[error_for(record) for record in failed],
    )
