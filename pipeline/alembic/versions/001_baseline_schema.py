"""Baseline schema: core tables plus the external-data cache tables.

This is the initial migration that captures the existing database schema, and
is the only definition of it. Running 'alembic upgrade head' on a fresh
database builds the schema from scratch.
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_uniprot_gene_symbol "
        "ON uniprot_info(gene_symbol)"
    )
    op.execute(
