...
import { contrast } from "./helpers/colour.ts";
 ...
 * `.card`": `.about-hero` restyles a surface without being a recipe member
 * itself, and is safe only because routes/index.tsx writes it as
 * `card about-hero` -- a pairing this stays false the moment anything drops,
 * instead of trusting a name list to still be true. (No modifier re-declares
 * wash or base today, so nothing needs the exemption; this is what keeps the
 * contract from failing the first time one does.)
 *
...
Deno.test("only sticky desktop filters receive the stacked-navbar cap", () => {
  ...
  // (width-independent) scroll-padding-top still reads the token below
  // that width.
  //
...
Deno.test("every surface that declares recipe parameters joins the recipe", () => {
  ...
  // written alongside, the shape `.pipeline-record-rejected` had in
  // PipelineRun.tsx while the drawer's records were still framed surfaces --
  // is the other way through, and it is checked rather
...
const SPACING_ALLOWLIST = new Map<string, string>([
  ...
  [
    ...
    ".pipeline-badge, .pipeline-chip, .about-row, .pipeline-note-subjects, " +
    ".pipeline-stat",
  ],
...
Deno.test("a tip box is a note, not a framed panel", () => {
  // It keeps the note grammar -- a rail on the reading edge, a wash and a
  // hairline -- and loses the four registration marks, which on a one-line
  // hint above a figure or a table read as the frame of a second figure.
  const members = recipeSelectorList();
  assert(!members.includes(".tip-box"), ".tip-box must not join the recipe");
  assert(!/\.tip-box::after/.test(css), "no corner marks");
  const rail = declarationsOf("border-left").find((d) =>
    d.selector === ".tip-box"
  );
  assertMatch(rail?.value ?? "", /^2px solid /, "the rail survives");
});

Deno.test("a drawer record is a row of its list, not a framed panel", () => {
  // Framed, every record drew its own box and four registration marks inside
  // the card's frame, and a stack of them piled crosses up between every
  // pair. They are rows now, ruled the way the Data Sources panel's are.
  assert(!recipeSelectorList().includes(".pipeline-record"));
  const rule = declarationsOf("border-top").find((d) =>
    d.selector === ".pipeline-record + .pipeline-record"
  );
  assertEquals(rule?.value, "1px solid var(--svd-border)");
});

Deno.test("every chip and badge is a pill", () => {
  // The system draws every object square, and the chips are the exception
  // by request: a capsule says "a value" where a square says "a panel". One
  // token rounds the whole family, so no chip can be left square by a rule
  // that forgot it.
  assertEquals(
    css.match(/--svd-radius-full:\s*([^;]+);/)?.[1].trim(),
    "999px",
  );
  const radii = declarationsOf("border-radius");
  for (
    const chip of [
      ".filter-count",
      ".date-badge",
    ...
    const rule = radii.find((d) =>
      d.selector.split(",").map((s) => s.trim()).includes(chip)
    );
    assertEquals(rule?.value, "var(--svd-radius-full)", chip);
  }
});

Deno.test("an HTTP method reads as code beside its path, not as a chip", () => {
  // `GET` in a tinted box and its path in plain mono read as two things;
  // the method and the endpoint are one line of code.
  const family = declarationsOf("background").find((d) =>
    d.value === "color-mix(in oklab, var(--svd-tint) 14%, transparent)"
  );
  assert(family !== undefined, "the chip family's wash is declared once");
  assert(
    !family.selector.split(",").map((s) => s.trim()).includes(
      ".pipeline-method",
    ),
    `the method is not a chip: ${family.selector}`,
  );
});

Deno.test("the figure key's toggle is filled, not a hairline box", () => {
  // Closed on load, the key is what a first-time reader has to find before
  // either plate means anything, and a hairline box beside a 1072px figure
  // read as chrome. It takes the login button's fill and ink -- the pair
  // measured at 9.08:1 -- and the same step on hover and on focus.
  const summary = ".figure-key > summary";
  assertEquals(
    declarationsOf("background").find((d) => d.selector === summary)?.value,
    "var(--svd-color-primary)",
  );
  assertEquals(
    declarationsOf("color").find((d) => d.selector === summary)?.value,
    "var(--svd-on-primary)",
  );
  assertEquals(
    declarationsOf("background").find((d) =>
      d.selector === `${summary}:hover, ${summary}:focus-visible`
    )?.value,
    "var(--svd-color-accent-text)",
  );
});
...
/** A token's value in the light root, or in the first dark block over it. */
function tokenValue(name: string, theme: "light" | "dark"): string {
  const declared = (text: string) =>
    new Map(
      [...text.matchAll(/^\s*(--svd-[\w-]+):\s*([^;]+);/gm)].reverse()
        .map((m) => [m[1], m[2].trim()] as const),
    );
  const light = declared(tokenRegion);
  const dark = css.match(/color-scheme:\s*dark;([\s\S]*?)\n\s*\}/)![1];
  const tokens = theme === "light"
    ? light
    : new Map([...light, ...declared(dark)]);
  let value = tokens.get(name);
  for (let hops = 0; value?.startsWith("var(") && hops < 8; hops++) {
    value = tokens.get(value.slice(4, -1));
  }
  assert(value !== undefined, `${name} does not resolve in ${theme}`);
  return value.replace(/\s*\/\*.*$/, "");
}

Deno.test("a form control's edge holds 3:1 against the page in both themes", () => {
  // WCAG 1.4.11: an empty field's box is all that says where the field is,
  // and --svd-line measured 1.36:1 (light) and 1.78:1 (dark).
  for (const theme of ["light", "dark"] as const) {
    const edge = tokenValue("--svd-line-control", theme);
    for (const behind of ["--svd-ground", "--svd-surface"]) {
      const ratio = contrast(edge, tokenValue(behind, theme));
      assert(ratio >= 3, `${theme} ${behind}: ${ratio.toFixed(2)}:1`);
    }
  }
  for (
    const rule of [
      /\.adapt-select \{[^}]*border: 1px solid var\(--svd-line-control\);/s,
      /\.login-form input\[type='password'\] \{[^}]*border: 1px solid var\(--svd-line-control\);/s,
    ]
  ) assertMatch(css, rule);
});

Deno.test("the wizard's stepper contains what it scrolls, and its ring", () => {
  // Unpositioned, the row was not the containing block of a step's
  // visually-hidden count, which escaped the scroller and widened the page
  // on a phone; e2e/tests/adapt_layout.spec.ts measures it.
  assertMatch(css, /\.adapt-stepper \{\s*position: relative;/);
  assertMatch(
    css,
    /\.adapt-stepper \{[^}]*overflow-x: auto;\s*padding: var\(--svd-space-1\);\s*margin: calc\(var\(--svd-space-1\) \* -1\);/s,
  );
});

Deno.test("the wizard looks inert until it is live, and a lookup's held button looks disabled", () => {
  assertMatch(css, /\.adapt:disabled > :not\(\.adapt-status\) \{\s*opacity:/);
  assertMatch(css, /\.adapt:disabled \* \{\s*cursor: progress;/);
  assertMatch(
    css,
    /\.adapt-button\[aria-disabled="true"\]:focus-visible \{\s*cursor: not-allowed;/,
  );
  // Spaced by gaps: a status paragraph keeps no global margin.
  assertMatch(css, /\.adapt-status \{\s*margin: 0;/);
});
