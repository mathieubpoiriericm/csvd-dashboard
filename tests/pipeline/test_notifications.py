...
from pipeline.notifications import (
    ...
    _format_duration,
    _make_send_notification,
    _render_markdown,
    send_pipeline_notification,
)
from pipeline.report import PaperSummary, PipelineRunData
...
def _make_run_data(
    *,
    mode: str | None = None,
    dry_run: bool = False,
    papers_detail: list[PaperSummary] | None = None,
    database: MergeResult | None = None,
...
@patch("pipeline.notifications.apprise.Apprise")
def test_a_render_failure_is_logged_rather_than_raised(mock_apprise_cls, caplog):
    """The docstring promises nothing propagates, and rendering is part of that.

    The render sat outside the try, so a template error -- `token_usage`
    missing, which the template compares against zero -- escaped. It ran
    after the `completed` row was written, so a merged run gained a second,
    `failed` row for a digest that was never going to matter.
    """
    mock_apprise_cls.return_value = MagicMock()
    run_data: Any = _make_run_data()
    del run_data["token_usage"]

    with caplog.at_level(logging.ERROR):
        send_pipeline_notification(run_data, _make_config())

    assert "Failed to send pipeline notification" in caplog.text


@pytest.mark.parametrize("outcome", [False, None], ids=["failed", "no-service"])
@patch("tenacity.nap.time.sleep")
@patch("pipeline.notifications.apprise.Apprise")
def test_a_delivery_failure_is_retried_and_logged(
    mock_apprise_cls, mock_sleep, outcome, caplog
):
    """Apprise reports failure through its return value, not an exception.

    ``notify()`` returns False when a service failed and None when no
    service could be loaded, and raises for neither -- so an exceptions-only
    retry never fired, the configured retries were dead, and the caller
    logged success over an undelivered digest.
    """
    mock_instance = MagicMock()
    mock_instance.notify.return_value = outcome
    mock_apprise_cls.return_value = mock_instance

    config = _make_config(notify_max_retries=3)
    ...
    assert "Failed to send pipeline notification" in caplog.text
    assert "Pipeline notification sent successfully" not in caplog.text


@patch("pipeline.notifications.apprise.Apprise")
def test_sender_registers_multiple_nonempty_urls(mock_apprise_cls):
    apprise = MagicMock()
    mock_apprise_cls.return_value = apprise
    config = _make_config(notify_urls="json://one, ,json://two")

    _make_send_notification(config)

    assert [call.args[0] for call in apprise.add.call_args_list] == [
        "json://one",
        "json://two",
    ]
