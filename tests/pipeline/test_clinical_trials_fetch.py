...
class TestIsDiseaseStudy:
    """The gate that keeps a curator's queue readable.

    `query.cond` expands into CT.gov's concept graph, and with no gate the
    most common stated condition across ten terms was a disease that merely
    causes the one the dashboard tracks. The gate reads the condition
    vocabulary off disease/pipeline.json; what the first disease's
    vocabulary keeps and drops is pinned in
    tests/pipeline/csvd/test_clinical_trials_fetch.py.
    """
    ...
    def test_a_stated_condition_substring_is_kept(self):
        d = load_disease()
        assert is_disease_study(self._study(d.ct_condition_substrings[0].upper()))

    def test_a_condition_pair_is_matched_inside_one_string(self):
        d = load_disease()
        first, second = d.ct_condition_pairs[0]
        assert is_disease_study(self._study(f"{first} something {second}"))
        assert not is_disease_study(self._study(first, second))

    def test_an_unrelated_condition_is_dropped(self):
        assert not is_disease_study(self._study("Common Cold"))

    def test_an_accented_condition_meets_its_unaccented_term(self, monkeypatch):
        # "Behçet's Disease" read as "beh et s disease", so a configured
        # "behcet" matched none of the registry's accented conditions; and a
        # comma left two spaces, so "stroke lacunar" missed "Stroke, Lacunar".
        import pipeline.clinical_trials_fetch as fetch

        monkeypatch.setattr(fetch, "_FOLDED_CONDITIONS", ("behcet", "stroke lacunar"))
        monkeypatch.setattr(fetch, "_FOLDED_PAIRS", ())
        assert is_disease_study(self._study("Behçet's Disease"))
        assert is_disease_study(self._study("Stroke, Lacunar"))
        assert not is_disease_study(self._study("Common Cold"))
        assert fetch._fold("Behçet's  Disease") == "behcet s disease"
        assert fetch._fold("Sjögren") == "sjogren"
