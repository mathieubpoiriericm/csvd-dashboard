...
class TestSearchRecentPapers:
    ...
    async def test_a_quoted_phrase_matching_nothing_is_logged(self, mocker, caplog):
        # Every term the query writes is quoted, and NCBI files a quoted
        # phrase with no hits under WarningList, not ErrorList -- the shape
        # Bio.Entrez returns for the live query. OutputMessage is commentary.
        mocker.patch(
            "pipeline.pubmed_search._run_entrez_search",
            new_callable=AsyncMock,
            return_value={
                ...
                "Count": "1",
                "WarningList": {
                    "QuotedPhraseNotFound": ['"nonesuch"[Title/Abstract]'],
                    "PhraseIgnored": ["of"],
        ...
        assert '"nonesuch"[Title/Abstract]' in caplog.text
        assert "ignored part of the query: of" in caplog.text
        assert "No items found." not in caplog.text
