...
def _keys_of(rows: list[dict[str, object]]) -> set[str]:
    return {key for row in rows for key in row}


def test_clean_column_name_then_to_camel_reproduces_table1_wire_keys(
    committed_rows,
) -> None:
    ...
    expected = _keys_of(committed_rows("table1.json")) - {"linkToMonogenicDisease"}
    assert produced == expected


def test_clean_column_name_then_to_camel_reproduces_table2_wire_keys(
    committed_rows,
) -> None:
    ...
    assert produced == _keys_of(committed_rows("table2.json"))
...
_RENAMED_AFTER_CLEAN_COLUMN_NAME = {
    ...
    # The column keeps its cSVD-era name; the header a reader sees is the
    # manifest's cellTypes.label, resolved through _MANIFEST_HEADERS above.
    "brain_cell_types": lambda s: _MANIFEST["cellTypes"]["label"],
}

# Columns whose header is a manifest label rather than the cleaned name; their
# accessor key is still to_camel of the cleaned name.
_MANIFEST_LABELLED_COLUMNS = frozenset({"brain_cell_types"})
...
def test_clean_column_name_matches_the_dashboards_committed_headers() -> None:
    ...
    for column in _GENES_TABLE_COLUMNS:
        ...
        # except for the column whose header is the manifest's label: the
        # wire key stays what the column name gives, whatever the label says.
        keyed_on = label if column in _MANIFEST_LABELLED_COLUMNS else expected_label
        key = to_camel(keyed_on)
