...
def test_the_gate_republishes_every_committed_value_unchanged(
    committed_rows,
) -> None:
    ...
    data/table1.json.
    """
    rows = committed_rows("table1.json")
    published = [
...
def test_gene_row_key_order_matches_the_committed_artifact(committed_rows) -> None:
    ...
    """
    expected = list(committed_rows("table1.json")[0].keys())
...
def test_trial_row_key_order_matches_the_committed_artifact(committed_rows) -> None:
    """Field order is part of write_rows' contract: pipeline/export/writer.py
    ...
    hard-coded, so drift in either direction gets caught.
    """
    expected = list(committed_rows("table2.json")[0].keys())
...
def test_clean_gene_row_folds_synonyms_and_drops_untracked_terms(
    trait_vocabulary,
) -> None:
    ...
    row = clean_gene_row(
        {
            "gene": "GENEA/B",
            "gwas_trait": [trait_vocabulary.untracked[0], spelling, other, target],
            "references": ["42437605"],
        }
    )
    assert row["GWAS Trait"] == [target, other]


def test_clean_gene_row_publishes_the_sentinel_when_only_untracked_terms_remain(
    trait_vocabulary,
) -> None:
    untracked = list(trait_vocabulary.untracked)
    row = clean_gene_row({"gene": "X", "gwas_trait": untracked, "references": ["1"]})
    assert row["GWAS Trait"] == ["(none found)"]


def test_split_gwas_traits_folds_synonyms_in_source_order(trait_vocabulary) -> None:
    ((spelling, target),) = trait_vocabulary.synonyms.items()
    other = trait_vocabulary.tracked[1]
    assert split_gwas_traits(f"{spelling}, {other}") == [target, other]
