...
class TestBuildGeneData:
    ...
    def test_gwas_trait_is_a_deduped_list(self, make_gene_entry, trait_vocabulary):
        """The join table takes one row per trait, so the merge stops joining."""
        a, b, c = trait_vocabulary.tracked
        data = _build_combined_gene_data(
            [
                make_gene_entry(gwas_trait=[a, b]),
                make_gene_entry(gwas_trait=[b, c]),
            ]
        )
        assert data["gwas_trait"] == [a, b, c]
    ...
    def test_an_untracked_trait_is_dropped_before_it_is_stored(
        self, make_gene_entry, caplog, trait_vocabulary
    ):
        ...
        tracked = trait_vocabulary.tracked[0]
        first, second = trait_vocabulary.untracked
        data = _build_combined_gene_data(
            [
                make_gene_entry(gene_symbol="GENEA", gwas_trait=[first, tracked]),
                make_gene_entry(gene_symbol="GENEA", gwas_trait=[second, tracked]),
            ]
        )
        assert data["gwas_trait"] == [tracked]
        assert "GENEA" in caplog.text
        assert first in caplog.text
        assert second in caplog.text

    def test_tracked_traits_are_kept_without_a_log_line(
        self, make_gene_entry, caplog, trait_vocabulary
    ):
        caplog.set_level(logging.INFO)
        a, b, _ = trait_vocabulary.tracked
        data = _build_combined_gene_data([make_gene_entry(gwas_trait=[a, b])])
        assert data["gwas_trait"] == [a, b]
        assert "untracked" not in caplog.text

    def test_a_prompt_synonym_is_folded_onto_its_trait(
        self, make_gene_entry, trait_vocabulary
    ):
        ...
        """
        ((spelling, target),) = trait_vocabulary.synonyms.items()
        other = trait_vocabulary.tracked[1]
        data = _build_combined_gene_data(
            [
                make_gene_entry(gwas_trait=[spelling, other]),
                make_gene_entry(gwas_trait=[target]),
            ]
        )
        assert data["gwas_trait"] == [target, other]
...
class TestMergeGeneEntries:
    ...
    async def test_a_low_confidence_update_carries_no_causal_claim(
        self, make_gene_entry, mocker, caplog, tracked_traits
    ):
    ...
    async def test_a_confident_update_still_carries_its_claims(
        self, make_gene_entry, mocker, tracked_traits
    ):
        ...
        assert row["gwas_trait"] == [tracked_traits[0]]
    ...
    async def test_deduplicates_within_batch(
        ...
        assert a in merged["gwas_trait"]
        assert b in merged["gwas_trait"]
...
class TestCanonicalGeneSymbol:
    ...
    def test_both_members_map_to_the_curated_key(self, gene_aliases):
        assert canonical_gene_symbol("GENEA") == "GENEA/B"
        assert canonical_gene_symbol("GENEB") == "GENEA/B"

    def test_matching_ignores_case_and_surrounding_space(self, gene_aliases):
        assert canonical_gene_symbol(" genea ") == "GENEA/B"

    def test_the_curated_key_is_stable_under_a_second_pass(self, gene_aliases):
        assert canonical_gene_symbol("GENEA/B") == "GENEA/B"

    def test_every_other_symbol_is_returned_untouched(self, gene_aliases):
        """A no-op for all but the pairs the curators combine, so this
        cannot quietly rewrite the other genes."""
        for symbol in ("NOTCH3", "HTRA1", "LAMB1", "COL4A3", "COL6A1"):
            assert canonical_gene_symbol(symbol) == symbol

    def test_a_mixed_case_member_folds_onto_its_key(self, monkeypatch):
        """HGNC writes an open-reading-frame symbol in mixed case.

        The lookup is upper-cased, so a member keyed as written -- C9orf72 --
        was never found, and its extractions became a row of their own beside
        the curated key the map exists to keep them under.
        """
        ...
        monkeypatch.setattr(
            merger,
            "_CANONICAL_GENE_SYMBOLS",
            reverse_aliases({"C1orf999/GENEX": ("C1orf999", "GENEX")}),
        )
        for symbol in ("C1orf999", "C1ORF999", " c1orf999 ", "GENEX"):
            assert canonical_gene_symbol(symbol) == "C1orf999/GENEX", symbol

    def test_an_obsolete_curated_symbol_keeps_its_row(self, gene_aliases):
        """NCBI answers cSVD's C6orf195 under its current name, LINC01600.
        ...
        a second form.
        """
        assert canonical_gene_symbol("NEWSYM") == "OLDSYM"
        assert canonical_gene_symbol("newsym") == "OLDSYM"
    ...
    def test_every_renamed_curated_gene_has_a_map_entry(self):
        ...
        that the committed data shows the case at all is pinned in
        tests/pipeline/csvd/test_data_merger.py.
        """
    ...
    def test_the_stored_row_carries_the_curated_key(
        self, make_gene_entry, gene_aliases
    ):
        data = _build_combined_gene_data([make_gene_entry(gene_symbol="GENEA")])
        assert data["gene"] == "GENEA/B"

    async def test_both_members_in_one_batch_collapse_into_one_row(
        self, make_gene_entry, mocker, gene_aliases
    ):
        ...
        await merge_gene_entries(
            [
                make_gene_entry(gene_symbol="GENEA", pmid="11111111"),
                make_gene_entry(gene_symbol="GENEB", pmid="22222222"),
            ]
        )
    ...
    async def test_a_new_member_does_not_create_a_second_row(
        ...
        await merge_gene_entries([make_gene_entry(gene_symbol="GENEA")])

        to_insert, to_update = merge.call_args[0]
        assert to_insert == []
        assert [row["gene"] for row in to_update] == ["GENEA/B"]
