...
def _make_entry(
    ...
    pmid: str = "12345678",
    source_quote: str = "NOTCH3 variants were associated with WMH (p=1e-12).",
) -> GeneEntry:
    return GeneEntry(
...
class TestPanderaSchema:
    ...
    def test_empty_gene_symbol_caught(self):
        # GeneEntry refuses a blank symbol at validation now, so the schema
        # is the second net; build the entry without validation to reach it.
        genes = [_make_entry().model_copy(update={"gene_symbol": ""})]
        warnings = batch_validate(genes)
...
class TestGeneDuplication:
    ...
    def test_entry_without_pmid_is_not_counted(self):
        warnings = batch_validate([_make_entry(pmid="")])

        assert not any("extracted from" in warning for warning in warnings)
