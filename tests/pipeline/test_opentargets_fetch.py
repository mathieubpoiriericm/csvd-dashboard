...
class TestSyncOpenTargetsAnnotations:
    ...
    async def test_the_meta_query_runs_once_for_the_whole_run(self, mocker) -> None:
        ...
        mocker.patch(
            ...
            AsyncMock(
                side_effect=[
                    ...
                    # COL4A1/2 is not a gene symbol, so it is queried as
                    # its two member chains -- two calls, one status row.
                    # An unresolvable symbol is a finding, not a failure.
                    OpenTargetsTarget(
                        gene_symbol="COL4A1",
                        ensembl_id=None,
                        rows=(),
        ...
        result = await sync_opentargets_annotations(["HTRA1", "COL4A1/2", "FOXF2"])
