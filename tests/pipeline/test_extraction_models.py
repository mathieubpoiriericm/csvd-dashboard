...
def test_a_prompt_spelling_of_a_tracked_trait_is_admitted() -> None:
    """The prompt asks for `cerebral-microbleeds`; the schema must not refuse it.
    ...
    the spelling onto CMB; the schema's job is only to admit it.
    """
    entry = GeneEntry(
        gene_symbol="NOTCH3",
        confidence=0.9,
...
def test_a_curated_spelling_is_not_admitted() -> None:
    """`small vessel stroke` is a spreadsheet spelling the prompt never asks for."""
    with pytest.raises(ValidationError):
        GeneEntry(
            gene_symbol="NOTCH3",
            confidence=0.9,
