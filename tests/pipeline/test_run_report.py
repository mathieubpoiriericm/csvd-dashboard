...
class TestCounts:
    ...
    def test_a_held_gene_is_matched_under_its_canonical_key(self) -> None:
        # `merge_gene_entries` groups on the canonical key, so a held
        # COL4A1 is recorded as `COL4A1/2`. The extracted symbol has to go
        # through the same mapping, or the gene is listed under both
        # accepted and rejected -- the double count this exclusion exists
        # to remove.
        report = _report(
            database={
                "inserted": 0,
                "updated": 0,
                "held_below_insert_floor": [
                    {"gene_symbol": "COL4A1/2", "confidence": 0.6}
                ],
            },
            papers_detail=[
                {
