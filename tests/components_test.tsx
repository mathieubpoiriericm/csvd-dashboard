...
Deno.test("presentational shells render their optional variants", () => {
  ...
  assertStringIncludes(html, `alt="${escapeHtml(INSTITUTE.logo.alt)}"`);
...
Deno.test("InstituteLogo renders one file for a named theme", () => {
  ...
  assertStringIncludes(light, `src="${escapeHtml(INSTITUTE.logo.src)}"`);
  assertStringIncludes(light, `alt="${escapeHtml(INSTITUTE.logo.alt)}"`);
  assertEquals((light.match(/<img/g) ?? []).length, 1);

  const dark = renderToString(<InstituteLogo theme="dark" />);
  assertStringIncludes(
    dark,
    `src="${escapeHtml(INSTITUTE.logo.srcOnDark ?? INSTITUTE.logo.src)}"`,
  );
...
Deno.test("InstituteLogo defaults to the pair the stylesheet picks between", () => {
  ...
  assertStringIncludes(
    html,
    `class="institute-logo institute-logo-light" src="${
      escapeHtml(INSTITUTE.logo.src)
    }"`,
  );
  assertStringIncludes(
    html,
    `class="institute-logo institute-logo-dark" src="${
      escapeHtml(INSTITUTE.logo.srcOnDark ?? INSTITUTE.logo.src)
    }"`,
  );
  const alt = escapeHtml(INSTITUTE.logo.alt).replace(
    /[.*+?^${}()|[\]\\]/g,
    "\\$&",
  );
  assertEquals(
    (html.match(new RegExp(`alt="${alt}"`, "g")) ?? []).length,
    2,
  );
...
Deno.test("MapPopup deduplicates trial facts and renders facility details", () => {
  const html = render(
    <MapPopup
      location={LOCATION}
      trials={[
        { ...sampleTrial(), drug: " Drug A ", targetSampleSize: "100" },
        {
          ...sampleTrial(),
...
Deno.test("MapPopup uses singular labels for one trial", () => {
  const html = render(
    <MapPopup
      location={LOCATION}
      trials={[{ ...sampleTrial(), drug: "Drug A", targetSampleSize: "100" }]}
    />,
  );
...
Deno.test("TableShell tags every cell with its column id", () => {
  const html = render(<GenesView rows={[sampleGene()]} />);
...
Deno.test("GenesView: absent values render as an em dash, not the export sentinel", () => {
  const html = render(
    <GenesView
      rows={[
        sampleGene(),
        sampleGene({ gene: "GENE2", affectedPathway: "NA", gwasTrait: ["T1"] }),
      ]}
    />,
  );
...
Deno.test("BrainCellTypesCell renders an em dash for the absent sentinel, before splitting", () => {
  ...
  // The abbreviations are the manifest's glossary, whatever it holds.
  const [known, second] = Object.keys(CELL_TYPE_NAMES);
  const present = render(<BrainCellTypesCell value={`${known}>${second}`} />);
  assertStringIncludes(present, known);
  assertStringIncludes(present, second);
  ...
  const mixed = render(<BrainCellTypesCell value={`${known}>QQ`} />);
  assertStringIncludes(mixed, known);
...
Deno.test("GenesView: a GWAS trait choice carries its long name as a visually-hidden description", () => {
  ...
  assertStringIncludes(
    html,
    `class="visually-hidden">, ${escapeHtml(vocabulary.traits[0].name)}`,
  );
});
...
Deno.test("TrialsView: every filter group's legend is its one true name", () => {
  ...
  assertEquals(legends, [
    ...
    escapeHtml(POPULATION_FIELD.label),
    "Sponsor Type",
    "Study status",
  ]);
});
