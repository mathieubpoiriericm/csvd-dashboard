...
import { INSTITUTE, SITE_TITLE } from "../lib/disease.ts";
...
Deno.test("the login card shows the institute logo, the form and the return path", () => {
  ...
  assertStringIncludes(html, escapeHtml(SITE_TITLE));
...
Deno.test("the shell renders the login page without the navbar", () => {
  ...
  assertStringIncludes(
    html,
    `<title>Sign in | ${escapeHtml(SITE_TITLE)}</title>`,
  );
