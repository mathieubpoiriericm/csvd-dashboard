...
export function geneTooltip(geneSymbol: string): TooltipContent | null {
  return geneInfoTooltip(geneInfoByName.get(geneSymbol));
}

/** UniProt summary, looked up by the gene symbol rather than the protein name. */
export function proteinTooltip(geneSymbol: string): TooltipContent | null {
  return proteinInfoTooltip(proteinInfoByGene.get(geneSymbol));
}
...
/** Formatted citation for a PMID. */
export function referenceTooltip(pmid: string): TooltipContent | null {
  const id = pmid.trim();
  const reference = referenceByPmid.get(id);
