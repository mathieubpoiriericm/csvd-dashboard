 ...
 *
 * Mirrors `lib/data/normalize.ts`, through the helpers both import from
 * `lib/normalize.ts` (never from `lib/data/`, whose every module is
 * protected): strings are trimmed and an optional string that is empty
 * becomes null. The one difference is failure: a required key that is
 * missing or a `schemaVersion` this code does not know throws at module
 * load. A manifest is authored, not fetched, so a wrong one is a build
 * error rather than a data gap to paper over.
 */
...
import { list, nullableText, record } from "../normalize.ts";
