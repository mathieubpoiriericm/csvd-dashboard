...
/** Trials per phase, unfiltered, in registry order with the odd phases last. */
function phaseBuckets(rows: readonly Trial[]) {
  const perPhase = countBy(rows, (trial) => trial.clinicalTrialPhase.trim());
  const phases = [
    ...PHASE_ORDER.filter((phase) => perPhase.has(phase)),
    ...[...perPhase.keys()]
      .filter((phase) => !PHASE_ORDER.includes(phase))
      .sort(),
  ];
  return { perPhase, phases };
}
...
interface TrialsViewProps {
  /** The rows to render; the committed table by default. Tests hand in synthetic rows. */
  rows?: readonly Trial[];
}

export default function TrialsView({ rows = trials }: TrialsViewProps = {}) {
  const { values, controls, summary, reset } = useCheckboxFilters(FILTERS);
  const buckets = useMemo(() => phaseBuckets(rows), [rows]);
  ...
  const readout = buckets.phases.map((phase) => ({
    label: phase || "—",
    total: buckets.perPhase.get(phase) ?? 0,
