...
import { protectedDataChunkName } from "./server/protected_data_assets.ts";
import { protectedDataBuildGuard } from "./server/protected_data_build.ts";
    ...
    client: {
      build: {
        rollupOptions: {
          output: {
            manualChunks(id) {
              return protectedDataChunkName(id, REPOSITORY_ROOT);
            },
          },
        },
      },
