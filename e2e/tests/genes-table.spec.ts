...
import { EXPECTED, GENE_TOTAL, pagesFor } from "../fixtures/expected-data.ts";
import { banner, pageSizeSelect, rows } from "../helpers.ts";

const TOTAL = GENE_TOTAL;
const PAGES = pagesFor(TOTAL);
/** The pager shows at most seven numbered buttons at once. */
const PAGE_WINDOW = 7;
const HAS_ROWS = TOTAL > 0;
...
test("loads one page of the committed gene set", async ({ page }) => {
  ...
  if (HAS_ROWS) {
    await expect(rows(page)).toHaveCount(Math.min(10, TOTAL));
    await expect(page.locator(".pagination span").first())
      .toHaveText(`Showing 1–${Math.min(10, TOTAL)} of ${TOTAL}`);
  } else {
...
test("the sticky header stays pinned while the body scrolls, and the two header rows stay adjacent", async ({ page }) => {
  test.skip(TOTAL < 20, "too few committed rows to scroll the table");
  // Every row on one page guarantees .table-scroll has more content than
  // its bounded height, regardless of how tall any individual row's content
  // happens to render.
  await pageSizeSelect(page).selectOption("100");
  await expect(rows(page)).toHaveCount(Math.min(TOTAL, 100));
...
test("clicking a header cycles the sort and reorders rows", async ({ page }) => {
  test.skip(!HAS_ROWS, "no committed rows to sort");
  ...
  await expect(firstCell).toHaveText(EXPECTED.firstGeneAsc!, {
    useInnerText: true,
  });

  await page.keyboard.press("Enter");
  await expect(gene).toHaveAttribute("aria-sort", "descending");
  await expect(firstCell).toHaveText(EXPECTED.firstGeneDesc!, {
    useInnerText: true,
  });
});

test("the page-size select repaginates", async ({ page }) => {
  test.skip(!HAS_ROWS, "no committed rows to paginate");
  await pageSizeSelect(page).selectOption("50");
  await expect(rows(page)).toHaveCount(Math.min(50, TOTAL));
  await expect(page.locator(".pagination span").first())
    .toHaveText(`Showing 1–${Math.min(50, TOTAL)} of ${TOTAL}`);

  test.skip(TOTAL > 100, "more than one page at the largest page size");
  await pageSizeSelect(page).selectOption("100");
...
test("pagination walks the pages and tracks the current one", async ({ page }) => {
  test.skip(
    PAGES <= PAGE_WINDOW,
    "the pager only slides once there are more pages than buttons",
  );
  const previous = page.getByRole("button", { name: "Previous" });
  const next = page.getByRole("button", { name: "Next" });

  // More pages than `pageWindow`'s seven buttons, so page 1 shows 1-7 and
  // the window slides as you walk right.
  await expect(numberedPages(page)).toHaveText(
    Array.from({ length: PAGE_WINDOW }, (_, i) => String(i + 1)),
  );
  ...
  // The last button in the window is not the last page, so the window
  // slides to reach it.
  await page.getByRole("button", { name: `Page ${PAGE_WINDOW}`, exact: true })
    .click();
  await expect(currentPage(page)).toHaveText(String(PAGE_WINDOW));
  await expect(next).toBeEnabled();

  await page.getByRole("button", { name: `Page ${PAGES}`, exact: true })
    .click();
  await expect(currentPage(page)).toHaveText(String(PAGES));
  // The last page holds the remainder.
  const remainder = TOTAL - (PAGES - 1) * 10;
  await expect(rows(page)).toHaveCount(remainder);
  await expect(page.locator(".pagination span").first())
    .toHaveText(`Showing ${(PAGES - 1) * 10 + 1}–${TOTAL} of ${TOTAL}`);
  await expect(next).toBeDisabled();

  await previous.click();
  await expect(currentPage(page)).toHaveText(String(PAGES - 1));
});

test("search narrows the table to one gene and clearing restores it", async ({ page }) => {
  test.skip(!HAS_ROWS, "no committed rows to search");
  ...
  // The symbol may also appear inside another row's prose; the first row is
  // the gene itself.
  await expect(rows(page).first().locator("td").first())
    .toHaveText(target, { useInnerText: true });
  await expect(banner(page)).toContainText(`Search: “${target}”`);
...
test("a query matching nothing shows the empty state", async ({ page }) => {
  // A nonsense string, deliberately: a real word may turn up somewhere once
  // Source Quote joined the searchable columns.
  ...
  await expect(rows(page)).toHaveCount(Math.max(1, Math.min(10, TOTAL)));
...
test("chromosomal location sorts by chromosome number", async ({ page }) => {
  ...
  expect(chromosomes[0]).toBe(Math.min(...chromosomes));
});
