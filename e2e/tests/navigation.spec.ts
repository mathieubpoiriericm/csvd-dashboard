...
test("the shared footer renders on every page", async ({ page }) => {
  ...
  await expect(footer).toContainText(
    `${INSTITUTE_COPYRIGHT}. All rights reserved.`,
  );
...
test("every font the shell preloads is consumed by a gated page", async ({ page }) => {
  await page.goto("/");
  const fonts = await preloadedFonts(page);
  // Body, heading and navbar faces: the three that render above the fold.
