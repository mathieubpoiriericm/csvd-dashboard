 ...
 * excluded. See e2e/fixtures/expected-data.ts for the numbers — regenerating
 * the data can legitimately change these.
 */
const TOTALS = [
  ["Putative Causal Genes", String(EXPECTED.genes)],
  ["Drugs Tested", String(EXPECTED.drugs)],
  ["Clinical Trials", String(EXPECTED.trials)],
  ["Publications", String(EXPECTED.publications)],
] as const;
...
test("reports the last run exactly once", async ({ page }) => {
  ...
  // one page would invite them to disagree.
  expect(hasWidget + await card.count()).toBe(1);
...
test("dates the data from the recorded run", async ({ page }) => {
...
test("lists the five info rows", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: "mathieu.poirier@icm-institute.org" }),
  )
    .toHaveAttribute("href", "mailto:mathieu.poirier@icm-institute.org");
});
