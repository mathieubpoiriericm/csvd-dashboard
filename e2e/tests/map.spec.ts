...
/**
 * The marker tests need a facility on the plate. On the empty export a fork
 * starts from there is none, so they skip rather than drill for a marker
 * that never comes; they return the moment the fork has geocoded trials.
 */
const NO_SITES = EXPECTED.map.sitesShown === 0;
const NO_MARKER = "no facility marker on the map";
...
test("reports the geocoded coverage", async ({ page }) => {
  ...
  await expect(stats.nth(0)).toContainText(`${EXPECTED.map.sites}`);
  await expect(stats.nth(0)).toContainText("sites");
  await expect(stats.nth(1)).toContainText(`${EXPECTED.map.countries}`);
  await expect(stats.nth(1)).toContainText("countries");
  await expect(stats.nth(2)).toContainText(`${EXPECTED.map.trials}`);
...
test.describe("dark theme", () => {
  ...
  test("the trial popup uses a dark surface", async ({ page }) => {
    test.skip(NO_SITES, NO_MARKER);
    // Wait for the container as the sibling tests do. Hydrating the
...
test("a hidden list carries the map's content for screen readers", async ({ page }) => {
  ...
  await expect(items).toHaveCount(EXPECTED.map.sitesShown);
...
async function openAnyMarker(page: import("@playwright/test").Page) {
  ...
  // yet, drawn as `d="M0 0"`. With many facility sites visible one of those is
...
test("the study-status group hides completed sites by default and shows them on request", async ({ page }) => {
  ...
  await expect(count).toContainText(
    `Showing ${EXPECTED.map.sitesShown} of ${EXPECTED.map.sites} sites`,
  );
  await page.getByRole("group", { name: /^Study status/ })
    .getByRole("checkbox", { name: /^Show All/ }).check();
  await expect(count).toContainText(
    `Showing ${EXPECTED.map.sites} of ${EXPECTED.map.sites} sites`,
  );
